...
st.set_page_config(page_title="UAV Pilot Cabinet v9.1", layout="wide", page_icon="🛡️")
...
MOTIVATION_MSGS = [
    ...
    "Чудова робота, пілоте! Дані успішно занесені в архів.",
    ...
    "Ваш професіоналізм — запорука безпеки нашої держави!"
]
...
if 'uploader_key' not in st.session_state: st.session_state.uploader_key = 100
...
def add_to_history(key, value):
    if value and value.strip() and value not in st.session_state.history[key]:
        ...
        st.session_state.history[key] = st.session_state.history[key][:10]
...
def send_telegram_master(all_fl):
    ...
    if all_media:
        ...
        for i, img in enumerate(all_media[:10]):
            file_key = f"photo{i}"
            media_group.append({
                "type": "photo", "media": f"attach://{file_key}",
                "caption": report if i == 0 else "", "parse_mode": "Markdown"
            })
...
if not st.session_state.splash_done:
    ...
    with container.container():
        ...
        my_bar = st.progress(0, text="Завантаження конфігурації v9.1...")
...
if not st.session_state.logged_in:
    ...
    with st.container(border=True):
        if role == "Пілот":
            ...
            h_name = st.session_state.history['name']
            n_sel = st.selectbox("Варіанти з історії (Прізвище):", ["-- Ввести нове --"] + h_name) if h_name else None
    ...
    with tab_f:
        ...
        with st.container(border=True):
            ...
            # ПАМ'ЯТЬ СЕСІЇ ДЛЯ ДАТИ ТА ЧАСУ
            m_date = c1.date_input("Дата польоту (дд.мм.рррр)", value=st.session_state.get('s_date', datetime.now()), key='s_date')
            ...
            r_sel = st.selectbox("Історія маршрутів:", ["-- Новий --"] + h_route)
            m_route = c4.text_input("Маршрут завдання:", value=r_sel if r_sel != "-- Новий --" else "", key='c_route')
            
            u_d = get_unit_drones(st.session_state.user['unit'])
            d_o = [f"{d['Модель БпЛА']} (s/n: {d['s/n']})" for d in u_d] if u_d else BASE_DRONES
            st.selectbox("🛡️ БпЛА НА ЗМІНУ (з серійним номером):", d_o, key='s_drone')

        with st.expander("➕ ДОДАТИ НОВИЙ ВИЛІТ", expanded=True):
            rt = st.session_state.reset_trigger
            col1, col2, col3, col4 = st.columns(4)
            t_o_s = col1.text_input("Зліт", key=f"zlit_{rt}", value="", placeholder="0900")
            t_l_s = col2.text_input("Посадка", key=f"pos_{rt}", value="", placeholder="0930")
            ...
            f_dist = col4.number_input("Відстань (м)", min_value=0, key=f"dst_{rt}", value=0)
            ...
            h_note = st.session_state.history['note']
            n_sel = st.selectbox("Історія приміток:", ["-- Нова --"] + h_note)
            f_note = st.text_area("Примітки", key=f"not_{rt}", value=n_sel if n_sel != "-- Нова --" else "", placeholder="Напр-д: Польоти не здійснювались, у зв'язку з несприятливими погодними умовами...")
            f_imgs = st.file_uploader("📸 Скріншоти", accept_multiple_files=True, key=f"upl_{st.session_state.uploader_key}")
        ...
        if st.session_state.temp_flights:
            ...
            st.dataframe(df_curr[["Зліт", "Посадка", "Тривалість (хв)", "Номер АКБ"]], use_container_width=True)
            ...
            if b3.button("🚀 ВІДПРАВИТИ ВСІ ДАНІ"):
                db_m = load_data("Sheet1"); final_to_db = []
    ...
    with tab_app:
        ...
        st.markdown("<div class='disclaimer'>⚠️ УВАГА: Даний розділ не відправляє заявки на ЦУС офіційно. Він призначений для швидкої генерації тексту заявки для подальшого копіювання в месенджери.</div>", unsafe_allow_html=True)
        with st.container(border=True):
            a_u = st.selectbox("1. Заявник:", UNITS, index=UNITS.index(st.session_state.user['unit']) if st.session_state.user['unit'] in UNITS else 0)
            ...
            if u_db and sel_f:
                m_list = list(set([s.split(" (s/n:")[0] for s in sel_f]))
                s_list = [s.split("s/n: ")[1].replace(")", "") for s in sel_f]
                app_sn = ", ".join(s_list); app_models = ", ".join(m_list)
            else:
                app_sn = st.text_input("s/n (якщо немає в базі):"); app_models = ", ".join(sel_f)
            ...
            h_route = st.session_state.history['route']
            ar_sel = st.selectbox("Раніше введені маршрути:", ["-- Новий --"] + h_route)
            ...
            h_phone = st.session_state.history['phone']
            p_sel = st.selectbox("Історія контактів:", ["-- Новий --"] + h_phone)
    ...
    with tab_info:
        ...
        with st.expander("🛡️ ІНСТРУКЦІЯ КОРИСТУВАЧА"):
            ...
            **2. 🚀 Вкладка «Польоти»**
            * **Крок А (Завдання):** Встановіть Дату, Час зміни та оберіть БпЛА на зміну. Дані збережуться до кінця сесії.
            * **Крок Б (Виліт):** Вкажіть час Зльоту/Посадки, Відстань, Номер АКБ та Цикли. Оберіть Результат, додайте Примітки та Скріншоти.
            * **Крок В (Управління):** Тисніть «✅ Додати у список». В кінці зміни обов'язково — «🚀 ВІДПРАВИТИ ВСІ ДАНІ».

            **3. 📡 Вкладка «ЦУС»**
            * Система сама розбиває польоти на вікна «До 00:00» та «Після 00:00». Просто копіюйте текст.
            """)
