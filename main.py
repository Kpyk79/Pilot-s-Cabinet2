...
st.set_page_config(page_title="UAV Pilot Cabinet v6.6", layout="wide", page_icon="🛡️")
...
if not st.session_state.logged_in:
    ...
    # --- ВКЛАДКИ ЗАЯВКА, ПОЛЬОТИ, ЦУС, АРХІВ (Залишені без змін) ---
    with tab_app:
    ...
    # --- ВКЛАДКА ДЛЯ ДОВІДКИ (ПОВНІ ІНСТРУКЦІЇ) ---
    with tab_info:
        ...
        with c1:
            st.markdown("""<div class='contact-card'>
                ...
                <div class='contact-desc'>Питання тактики застосування, налаштування систем та спеціалізованого ПЗ БпАС.</div>
                <b>Олександр</b><br>+380502310609
                </div>""", unsafe_allow_html=True)
        with c2:
            st.markdown("""<div class='contact-card'>
                <div class='contact-title'>🔧 Технік-майстер</div>
                <div class='contact-desc'>Механічні пошкодження майна, ремонт корпусів, заміна пропелерів, збої апаратної частини.</div>
                <b>Сергій</b><br>+380997517054
                </div>""", unsafe_allow_html=True)
        with c3:
            st.markdown("""<div class='contact-card'>
                <div class='contact-title'>📦 Начальник складу</div>
                <div class='contact-desc'>Облік майна, оформлення актів списання, переміщення та передача обладнання між підрозділами.</div>
        ...
        with st.expander("🛡️ ІНСТРУКЦІЯ КОРИСТУВАЧА: КАБІНЕТ ПІЛОТА (V6.0)", expanded=False):
            st.markdown("""
            ...
            * Введіть **Звання та Прізвище** (або позивний).
            * Натисніть кнопку **«Увійти»**.
            * *Примітка:* Якщо у вас були невідправлені чернетки в хмарі, вони підтягнуться автоматично.

            **2. 📋 Вкладка «Заявка» (Перед виїздом)**
            Використовується для швидкого створення тексту заявки на польоти.
            * Перевірте підрозділ та оберіть **Типи БпЛА** (можна декілька).
            * Вкажіть **серійні номери (s/n)** через кому.
            * Оберіть **період дат** та **час роботи**.
            * Впишіть **Маршрут**, **Висоту** та **Радіус**.
            * Натисніть **«Сформувати текст заявки»**.
            * Скопіюйте готовий текст (кнопка у верхньому правому куті вікна з текстом) та вставте його у месенджер.

            **3. 🚀 Вкладка «Польоти» (Основна робота)**
            Тут відбувається основний облік ваших вильотів.
            
            **Крок А: Дані зміни (Польотне завдання)**
            * Встановіть **Дату завдання** та **Час зміни** (Зміна з / до).
            * Вкажіть **Маршрут** (загальний напрямок).
            * **Важливо:** Оберіть **БпЛА на зміну** (цей дрон буде записаний для всіх вильотів цієї сесії).
            
            **Крок Б: Додавання вильотів**
            * Вкажіть час **Взльоту** та **Посадки** (хвилини порахуються автоматично).
            * Введіть **Відстань** у метрах.
            * Вкажіть **Номер АКБ** та поточну кількість **Циклів**.
            * Оберіть **Результат** та додайте **Примітки**, при необхідності (наприклад, «Польоти не здійснювались через несприятливі погодні умови»).
            * Завантажте **Скріншоти або Фото** (можна декілька за раз).
            * Натисніть **«➕ Додати у список»**.
            
            **Крок В: Керування чернеткою**
            * **🗑️ Видалити останній:** якщо помилилися в останньому записі.
            * **💾 Зберегти в Хмару:** якщо зміна ще триває, натисніть цю кнопку. Тепер ви можете закрити додаток, і дані не зникнуть.
            * **🚀 ВІДПРАВИТИ ВСІ ДАНІ:** натисніть наприкінці зміни. Дані підуть у офіційний архів.

            **4. 📡 Вкладка «ЦУС» (Для чергових)**
            Тут автоматично формується текст для доповіді на ЦУС.
            * Додаток сам розділить ваші польоти на два вікна: **«До 00:00»** та **«Після 00:00»**.
            * Логіка інтелектуальна: якщо політ почався о 23:50, а закінчився о 00:15 — він і всі наступні вильоти автоматично підуть у друге вікно.
            * Просто натисніть **«Копіювати»** на потрібному вікні.

            **5. 📜 Архів та 📊 Аналітика**
            * **Архів:** Тут ви бачите журнал усіх ваших минулих польотів. Дані оновлюються миттєво після відправки.
            * **Аналітика:** Тут відображається ваш загальний наліт за кожен місяць у зручному форматі (ГГ:ХХ), кількість вильотів та затримань.

            **💡 Поради для пілота:**
            * **Нічна зміна:** Якщо зміна переходить через північ, дата в календарі «Польоти» має стояти та, якою зміна почалася.
            * **Відсутність мережі:** Якщо інтернет слабкий, заповнюйте дані та натискайте «Зберегти в Хмару».
            * **Фото:** Завантажуйте скріншоти та фото коли в «Примітках» вказуєте причини з яких не здійснювався політ.
            """)

        with st.expander("📲 ЯК ВСТАНОВИТИ «КАБІНЕТ ПІЛОТА» НА СМАРТФОН", expanded=False):
            st.markdown("""
            Це дозволить відкривати програму однією кнопкою без пошуку посилання та адресного рядка.

            **Для Android (Google Chrome)**
            1.  Відкрийте посилання на ваш додаток у браузері **Chrome**.
            2.  Натисніть на **три крапки ( ⋮ )** у верхньому правому куті.
            3.  Оберіть пункт **«Додати на головний екран»** (або «Встановити додаток»).
            4.  Підтвердіть дію. Тепер іконка з'явиться серед інших програм на вашому телефоні.

            **Для iPhone (Safari)**
            1.  Відкрийте посилання у стандартному браузері **Safari**.
            2.  Натисніть кнопку **«Поділитися»** (квадрат зі стрілкою вгору по центру внизу).
            3.  Прокрутіть меню вниз і оберіть **«Додати на початковий екран»**.
