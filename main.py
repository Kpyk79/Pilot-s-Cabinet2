...
st.set_page_config(page_title="UAV Pilot Cabinet v8.2", layout="wide", page_icon="🛡️")
...
def send_telegram_msg(all_fl):
    ...
    if all_media:
        media_group = []
        files = {}
        for i, img in enumerate(all_media[:10]): # TG limit is 10
            file_key = f"photo{i}"
            media_group.append({
                "type": "photo",
...
# ЧИТАТИ ОДИН РАЗ ПІСЛЯ ІНІЦІАЛІЗАЦІЇ session_state

qp = st.experimental_get_query_params()

if "last_unit" not in st.session_state:
    st.session_state.last_unit = qp.get("unit", [None])[0]

if "last_name" not in st.session_state:
    st.session_state.last_name = qp.get("name", [None])[0]
# --- 8. ІНТЕРФЕЙС ---
...
n = st.text_input(
    ...
    with tab_app:
        ...
        with st.container(border=True):
            ...
            if u_d and sel_full:
                ...
                app_sn = st.text_input("s/n (через кому):"); app_models = ", ".join(sel_full)
    ...
    with tab_f:
        ...
        with st.expander("➕ ДОДАТИ НОВИЙ ВИЛІТ", expanded=True):
            ...
            f_note = st.text_area("Примітки", key=f"note_{rt}", value="", placeholder="Напр-д: Польоти не здійснювались, у зв'язку з несприятливими погодними умовами...")
    ...
    with tab_hist:
        ...
        if not df_h.empty and "Оператор" in df_h.columns:
            ...
            if not p_df.empty:
                st.dataframe(p_df.sort_values(by="Дата", ascending=False), use_container_width=True)
    ...
    with tab_info:
        ...
        with c2:
            st.markdown("""<div class='contact-card'><div class='contact-title'>🔧 Технік-майстер</div><div class='contact-desc'>Механічні пошкодження майна, ремонт корпусів, збої апаратної частини.</div><b>Сергій</b><br>+380997517054</div>""", unsafe_allow_html=True)
        with c3:
            st.markdown("""<div class='contact-card'><div class='contact-title'>📦 Начальник складу</div><div class='contact-desc'>Облік майна, оформлення актів списання, переміщення та передача обладнання.</div><b>Ірина</b><br>+380667869701</div>""", unsafe_allow_html=True)
        st.write("---")
        ...
        with st.expander("🛡️ ІНСТРУКЦІЯ КОРИСТУВАЧА"):
            ...
            * Введіть Звання та Прізвище. Натисніть «Увійти».

            **2. 📋 Вкладка «Заявка»**
            * Оберіть Типи БпЛА, s/n підтягнеться автоматично. Оберіть дати та час.
            * Натисніть «Сформувати текст заявки» та скопіюйте його у месенджер.

            **3. 🚀 Вкладка «Польоти»**
            * **Крок А:** Встановіть Дату та Час зміни. Оберіть БпЛА.
            ...
            **4. 📡 Вкладка «ЦУС»**
            * Додаток сам розділить ваші польоти на вікна «До 00:00» та «Після 00:00». Просто скопіюйте текст.
            """)
