...
st.set_page_config(page_title="UAV Pilot Cabinet v7.9", layout="wide", page_icon="🛡️")
...
# Ініціалізація ключів для очищення форми "Додати новий виліт"
form_fields = ["f_off", "f_land", "f_dist", "f_akb", "f_cyc", "f_res", "f_note"]
for field in form_fields:
    if field not in st.session_state:
        if "time" in field or "note" in field or "akb" in field: st.session_state[field] = ""
        elif "dist" in field or "cyc" in field: st.session_state[field] = 0
        elif "res" in field: st.session_state[field] = "Без ознак порушення"
...
if not st.session_state.logged_in:
    ...
    with tab_f:
        ...
        with st.expander("➕ ДОДАТИ НОВИЙ ВИЛІТ", expanded=True):
            ...
            # Введення часу текстом
            t_o_s = col1.text_input("Взльот", value=st.session_state.f_off, key="input_off", placeholder="09:00 або 900")
            t_l_s = col2.text_input("Посадка", value=st.session_state.f_land, key="input_land", placeholder="09:30 або 930")
            ...
            if p_o and p_l:
                ...
                col3.warning("Формат часу")
            
            f_dist = col4.number_input("Відстань (м)", min_value=0, key="f_dist")
            cb1, cb2 = st.columns(2)
            f_akb = cb1.text_input("АКБ №", key="f_akb")
            f_cyc = cb2.number_input("Цикли", min_value=0, key="f_cyc")
            ...
            if st.button("✅ ДОДАТИ У СПИСОК"):
                if p_o and p_l:
                    ...
                    # ОЧИЩЕННЯ ПОЛІВ ВІКНА ПІСЛЯ ДОДАВАННЯ
                    st.session_state.f_off = ""
                    st.session_state.f_land = ""
        ...
        if st.session_state.temp_flights:
            ...
            st.dataframe(df_curr[["Взльот", "Посадка", "Тривалість (хв)", "Номер АКБ", "Результат"]], use_container_width=True)
    ...
    with tab_stat:
        ...
        if not df_s.empty and "Оператор" in df_s.columns and "Дата" in df_s.columns:
            ...
            if not df_s_p.empty:
                ...
                if not df_s_p.empty:
                    df_s_p['Рік'] = df_s_p['dt'].dt.year
                    df_s_p['Місяць_номер'] = df_s_p['dt'].dt.month
