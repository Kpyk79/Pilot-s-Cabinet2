...
st.set_page_config(page_title="UAV Pilot Cabinet v10.4", layout="wide", page_icon="🛡️")
...
if not st.session_state.splash_done:
    st.markdown("<div style='text-align:center; margin-top:20%;'><h1>🛡️ UAV CABINET</h1><p>ЗАВАНТАЖЕННЯ v10.4...</p></div>", unsafe_allow_html=True)
    my_bar = st.progress(0); [ (time.sleep(0.01), my_bar.progress(p+1)) for p in range(100) ]
    st.session_state.splash_done = True; st.rerun()

# --- 8. ІНТЕРФЕЙС ---
# Визначаємо індекс підрозділу для пам'яті
...
if not st.session_state.logged_in:
    ...
    with st.container(border=True):
        ...
        n_hist = st.selectbox("Оберіть з історії:", ["-- Нове введення --"] + h_names) if h_names else None
    ...
    with tab_f:
        ...
        with st.expander("➕ ДОДАТИ НОВИЙ ВИЛІТ", expanded=True):
            ...
            f_note = st.text_area("Примітки", key=f"n_{rt}", value=n_hist_note if n_hist_note != "-- Нова --" else "", placeholder="Напр-д: Польоти не здійснювались...")
            ...
            if st.button("✅ ДОДАТИ У СПИСОК") and p_z and p_p and parsed_d:
                ...
                st.session_state.temp_flights.append({
                    "Дата": parsed_d, "Час завдання": f"{t_s_raw} - {t_e_raw}", "Підрозділ": st.session_state.user['unit'], "Оператор": st.session_state.user['name'], "Дрон": sel_drone, "Маршрут": m_route, "Зліт": p_z.strftime("%H:%M"), "Посадка": p_p.strftime("%H:%M"), "Тривалість (хв)": dur, "Дистанція (м)": f_dst, "Номер АКБ": f_akb, "Цикли АКБ": f_cyc, "Результат": "Виконано", "Примітки": f_note, "files": f_imgs
                })
    ...
    with tab_app:
        ...
        with st.container(border=True):
            ...
            # НОВІ ПОЛЯ: Прізвище + Телефон для зворотного зв'язку
            ...
            p_h_choice = st.selectbox("Історія телефонів:", ["-- Новий --"] + h_p)
            app_p_val = st.text_input("Номер телефону для зворотного зв'язку:", value=p_h_choice if p_h_choice != "-- Новий --" else "", placeholder="+380...")
            
            if st.button("✨ СФОРМУВАТИ"):
                add_to_history('phone', app_p_val)
                ...
                st.code(f"ЗАЯВКА НА ПОЛІТ\n1. Заявник: в/ч 2196 ({a_u})\n2. Тип БпЛА: {sel_d}\n3. Дата польоту: {dt_str}\n4. Час: {a_t1}-{a_t2}\n5. Маршрут: {app_r}\n9. Контактна особа: {app_contact_name}, тел: {app_p_val}", language="text")
    ...
    with tab_stat:
        ...
        if not df_s.empty:
            ...
            if not df_p.empty:
                ...
                # ВИПРАВЛЕННЯ ValueError: безпечне групування
                df_p['Y'] = df_p['dt'].dt.year
                df_p['M'] = df_p['dt'].dt.month
                rs = df_p.groupby(['Y', 'M']).agg(Вильоти=('Дата', 'count'), Наліт_хв=('Тривалість (хв)', 'sum')).reset_index()
