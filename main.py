...
# --- 8. SPLASH SCREEN ---
# Набір коротких підбадьорюючих повідомлень ("печива з пророцтвом")
MOTIVATION_COOKIES = [
    "БпЛА вже прогріває двигуни. Пілот — заряджайся на перемогу!",
    "Сьогодні твій політ буде як по нотах — без фальшивих сигналів!",
    "Якщо вітер зустрічний — значить ти летиш у правильному напрямку.",
    "Антидрон вдома, а твій скіл — з тобою. Поїхали!",
    "Сигнал стабільний, настрій бойовий — гарного патрулювання!",
    "Батарея свіжа, як ранкова кава. Час злітати!",
    "Комунікація чиста, як совість пілота. Працюємо!",
    "Штучний інтелект допоможе, але керуєш — ти!",
    "Пам'ятай: будь-яка турбулентність — це досвід.",
    "Зліт — упевнений, посадка — м'яка. Решта — майстерність!",
]

if not st.session_state.splash_done:
    container = st.empty()
    with container.container():
        random_cookie = random.choice(MOTIVATION_COOKIES)
        st.markdown(
            f"<div class='splash-container'><h1 style='font-size: 4em;'>🛡️</h1><h1>UAV PILOT CABINET</h1><div class='slogan-box'>{random_cookie}</div></div>",
            unsafe_allow_html=True,
        )
    ...
    with tab_f:
        ...
        if st.session_state.temp_flights:
            ...
            if cb3.button("🚀 ВІДПРАВИТИ ВСІ ДАНІ"):
                ...
                # Показати випадкове підбадьорююче повідомлення після успішної відправки
                st.success("✅ Надіслано!")
                st.info(random.choice(MOTIVATION_COOKIES))
                st.rerun()
