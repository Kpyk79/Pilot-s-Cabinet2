...
st.set_page_config(page_title="UAV Pilot Cabinet v9.2", layout="wide", page_icon="🛡️")
...
# Історія для Autocomplete (dropdown)
...
# --- 5. РОБОТА З БАЗОЮ (З кешуванням для запобігання APIError 429) ---
...
if not st.session_state.splash_done:
    ...
    my_bar = st.progress(0, text="Завантаження систем v9.2...")
...
if not st.session_state.logged_in:
    ...
    with st.container(border=True):
        ...
        n_sel = st.selectbox("Оберіть з історії:", ["-- Нове --"] + h_names) if h_names else None
        n = st.text_input("Звання та Прізвище:", value=n_sel if n_sel and n_sel != "-- Нове --" else "")
    ...
    with tab_f:
        ...
        with st.container(border=True):
            ...
            r_sel = st.selectbox("Раніше введений маршрут:", ["-- Ввести новий --"] + h_routes)
            m_route = c4.text_input("Маршрут завдання:", value=r_sel if r_sel != "-- Ввести новий --" else "", key='cur_route')
        ...
        with st.expander("➕ ДОДАТИ НОВИЙ ВИЛІТ", expanded=True):
            ...
            t_o_s = col1.text_input("Зліт", key=f"z_{rt}", value="", placeholder="09:00")
            ...
            f_dist = col4.number_input("Відстань (м)", min_value=0, key=f"d_{rt}", value=0)
            
            cb1, cb2 = st.columns(2)
            f_akb = cb1.text_input("Номер АКБ", key=f"a_{rt}", value="")
            f_cyc = cb2.number_input("Цикли АКБ", min_value=0, key=f"c_{rt}", value=0)
    ...
    with tab_app:
        ...
        st.markdown("<div class='disclaimer'>⚠️ Даний розділ не відправляє заявки на ЦУС. Він призначений виключно для швидкої генерації тексту заявки для копіювання в месенджери.</div>", unsafe_allow_html=True)
        with st.container(border=True):
            ...
            ar_sel = st.selectbox("Минулі маршрути:", ["-- Новий --"] + h_r)
            ...
            p_sel = st.selectbox("Минулі контакти:", ["-- Новий --"] + h_p)
    ...
    # --- ВКЛАДКИ ЦУС, АРХІВ, АНАЛІТИКА (БЕЗ ЗМІН) ---
    with tab_cus:
    ...
    # --- ВКЛАДКА ДОВІДКА (ВІДНОВЛЕНО v7.1/8.2) ---
    with tab_info:
        ...
        with st.expander("🛡️ ІНСТРУКЦІЯ КОРИСТУВАЧА"):
            st.markdown("**1. 🔑 Вхід:** Оберіть Підрозділ та Прізвище. Система запам'ятає ваші дані.\n**2. 🚀 Польоти:** Встановіть зміну (вона збережеться). Додавайте вильоти, вони очищаються автоматично.\n**3. 📋 Помічник заявки:** Швидкий генератор тексту (не відправляє в ЦУС).\n**4. 📡 ЦУС:** Інтелектуальний поділ на два часових вікна.")
        with st.expander("📲 ЯК ВСТАНОВИТИ НА СМАРТФОН"):
            st.markdown("**Android (Chrome):** ⋮ -> Додати на головний екран.\n**iPhone (Safari):** Поділитися -> Додати на початковий екран.")
        st.write("---")
